...
# Addding Multiple Test Pages ###################################
page = st.sidebar.selectbox('Choose Page', ['Exposure Data', 'Macroeconomic Data', 'Flow Rates', 'Expected Credit Loss Summary'])
...
GDP[::-1].to_csv("./data/gdp.csv") # Reversing the dataframe for readanility in tthe app
...
if page == 'Exposure Data':
    st.subheader("Exposure Data")
    st.write(df)

elif page == 'Flow Rates':
    st.subheader("Resultant Flow Rates")
    st.write(flow_rate)

elif page == 'Macroeconomic Data':
    st.subheader("GDP Data")
    st.dataframe(gdp_data)
    st.subheader("GDP Ddata normalised to Z-scores")
    st.write(z_scores)

elif page == 'Expected Credit Loss Summary':
    st.subheader("Final ECL Eummary")
    st.write(final_df)
...
# col1 = st.line_chart(final_df_PD)
col1.line_chart(final_df_PD)
col2.write(final_df_PD['TTC PD'])
